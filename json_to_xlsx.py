...
def judge_key(info, keyword):
    regex = re.compile(keyword)
    if isinstance(info['title'],str) and regex.search(info['title']):
        return True
    return False
...
def write_list_info():
    ...
    for info in info_dict:
        if index >= 100000:
            ...
            worksheet.cell(1,0+1, 'journal/conference')
            worksheet.cell(1,1+1, 'journal/conference name')
            worksheet.cell(1,2+1, 'year')
            worksheet.cell(1,3+1, 'author')
            worksheet.cell(1,4+1, 'title')
            sheet_index += 1
        year = get_year(info['ConOrJou'])
        if not year:
            continue
        if(judge_key(info, keyword)):
